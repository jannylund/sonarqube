...
public class SchedulerImpl implements Scheduler, ProcessEventListener, ProcessLifecycleListener, AppStateListener {
  ...
  private void tryToStartHealthStateSharing() {
    System.out.println(
      "healthStateSharing == null" + (healthStateSharing == null) +
        " appState instanceof ClusterAppState=" + (appState instanceof ClusterAppState) +
        " ClusterSettings.isLocalElasticsearchEnabled(settings)=" + ClusterSettings.isLocalElasticsearchEnabled(settings));
