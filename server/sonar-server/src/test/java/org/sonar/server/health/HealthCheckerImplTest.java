...
public class HealthCheckerImplTest {
  ...
  public void check_returns_green_status_without_any_cause_when_there_is_no_HealthCheck() {
    HealthCheckerImpl underTest = new HealthCheckerImpl(webServer, new NodeHealthCheck[0], new ClusterHealthCheck[0]);
  ...
  public void checkNode_returns_causes_of_all_HealthChecks_whichever_their_status() {
    ...
    HealthCheckerImpl underTest = new HealthCheckerImpl(webServer, nodeHealthChecks, new ClusterHealthCheck[0]);
  ...
  @Test
  public void checkCluster_returns_GREEN_status_if_only_GREEN_statuses_returned_by_HealthChecks() {
    when(webServer.isStandalone()).thenReturn(false);
    List<Health.Status> statuses = IntStream.range(1, 1 + random.nextInt(20)).mapToObj(i -> GREEN).collect(Collectors.toList());
  ...
  @Test
  public void checkCluster_returns_YELLOW_status_if_only_GREEN_and_at_least_one_YELLOW_statuses_returned_by_HealthChecks() {
    when(webServer.isStandalone()).thenReturn(false);
    List<Health.Status> statuses = new ArrayList<>();
    Stream.concat(
      IntStream.range(0, 1 + random.nextInt(20)).mapToObj(i -> YELLOW), // at least 1 YELLOW
  ...
  @Test
  public void checkCluster_returns_RED_status_if_at_least_one_RED_status_returned_by_HealthChecks() {
    when(webServer.isStandalone()).thenReturn(false);
    List<Health.Status> statuses = new ArrayList<>();
    Stream.of(
      IntStream.range(0, 1 + random.nextInt(20)).mapToObj(i -> RED), // at least 1 RED
  ...
  @Test
  public void checkCluster_returns_causes_of_all_HealthChecks_whichever_their_status() {
    when(webServer.isStandalone()).thenReturn(false);
  ...
  @Test
  public void checkCluster_passes_set_of_NodeHealth_returned_by_HealthState_to_all_HealthChecks() {
    when(webServer.isStandalone()).thenReturn(false);
  ...
  private NodeHealth randomNodeHealth() {
    return newNodeHealthBuilder()
      ...
      .setDetails(newNodeDetailsBuilder()
        ...
        .setPort(random.nextInt(333))
        .setStarted(random.nextInt(444))
        .build())
      .build();
  }
