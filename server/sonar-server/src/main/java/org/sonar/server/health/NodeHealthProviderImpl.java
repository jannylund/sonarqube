...
public class NodeHealthProviderImpl implements NodeHealthProvider {
  ...
  public NodeHealthProviderImpl(Configuration configuration, HealthChecker healthChecker, Server server, System2 system2) {
    ...
    this.nodeDetails = newNodeDetailsBuilder()
      ...
      .setType(computeType(configuration))
  ...
  private static NodeDetails.Type computeType(Configuration configuration) {
    if (configuration.getBoolean(CLUSTER_SEARCH_DISABLED).orElse(false)) {
      return NodeDetails.Type.APPLICATION;
    }
    return NodeDetails.Type.SEARCH;
  }
  ...
  private static int computePort(Configuration configuration) {
    return configuration.getInt(CLUSTER_PORT)
      .orElseThrow(missingPropertyISE(CLUSTER_PORT));
  }
